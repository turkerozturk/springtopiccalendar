...
public interface EntryRepository extends JpaRepository<Entry, Long>, EntryRepositoryCustom {
    ...
    Page<Entry> findAllByOrderByDateMillisYmdDescIdDesc(Pageable pageable);
    Page<Entry> findByTopicIdOrderByDateMillisYmdDescIdDesc(Long topicId, Pageable pageable);
    ...
    @Query("""
    SELECT e FROM Entry e
    WHERE e.topic.category.id = :categoryId
    AND e.dateMillisYmd = :todayMillisYmd
    ORDER BY e.dateMillisYmd DESC
    """)
    List<Entry> findByCategoryIdAndStatusOfAllEntriesWithDateInterval(@Param("categoryId") Long categoryId, @Param("todayMillisYmd") Long todayMillisYmd);
